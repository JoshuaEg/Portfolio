...
(* Order of Operation, (Declare error)-> 
...
let rec type_of e (env : environment) = match e with 
                | Int(_) -> Int_Type
                | Bool(_) -> Bool_Type
...
let rec optimize e =  
  let rec constan_fold expr = match expr with
    | Int(x) -> Int(x)
    | Bool(x) -> Bool(x)
    | ID(x) -> ID(x)
    | Value -> Value
  ...
  let rec set_var x v p = (* x is the ID(s) *)
    match p with
    | ID(s) when (ID(s)) = x -> v
    ...
    |_ -> p
  and set_stmt x v t = match t with
    | NoOp -> NoOp
  ...
  let rec loop_fold ast = match ast with 
    | NoOp -> NoOp
  ...
  let rec branch_fold ast = match ast with 
    | NoOp -> NoOp
  ...
  let rec fold_vars ast l =
      let rec fold_confirm e = match e with 
        | Int(_) -> true
        | Bool(_) -> true 
        | ID(_) -> false
  ...
  let rec stmt_fold ast = match ast with
    | NoOp -> NoOp
...
let rec typecheck e = 
  let rec stmt_of e (env : environment) = match e with
    ...
    | Assign(s,t,exp) -> (*if (t = type_of exp env || type_of exp env = Unknown_Type(0) || t = Unknown_Type(0))  then *)
...
let rec infer e =
  let rec type_env e env v = match e with
    ...
    |Binop(x,y,z) -> (match x with
          ...
          | Greater | GreaterEqual | Less | LessEqual | Equal | NotEqual -> 
            (match type_env y env v, type_env z env v with
              ...
              |_->[]      ))
    |_-> [] in
  let rec still_unknown e = match e with
    ...
    |_-> [] in
  let rec infer_exp e env v = match e with
  ...
  let rec inferencer e env = match e with
    | Assign(s,ty,exp) -> let expty = type_of exp env in (match ty with
                  | Unknown_Type(_) ->  if (List.assoc_opt s env) = None then
                                ...
                                |_-> (match expty with
                                  ...
                                  | Unknown_Type(_)-> env )                      )
                  |Int_Type -> let env = (match exp with
    ...
    | For(s,e1,e2,x)-> let env = (try (match List.assoc s env with 
                    ...
                    inferencer x (v2)
    | _ -> env in
  let rec updater e env = match e with
        ...
        |_-> e, [] in
  let rec updater2 e (z,ty) = match e with
  ...
  let _ = declare_check e in
  let env = inferencer e [] in let ast, z1 = updater e env in
...
(* Write a function that returns the type of an unknown value varible by scanning the rest of itself*)